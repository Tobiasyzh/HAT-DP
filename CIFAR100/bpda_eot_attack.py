...
class BPDA_EOT_Attack():
    ...
    def attack_batch(self, X, y):
        ...
        for step in range(self.config['adv_steps'] + 1):
            # ``defended`` is cumulative by design: verification can change a
            # surviving sample from True to False, but never restores a sample
            # that an earlier attack state already broke. The caller may
            # therefore report robustness directly from class_batch[-1].
